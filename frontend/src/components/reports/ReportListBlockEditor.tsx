...
export function ReportListBlockEditor({ block, selected, onSelect, onChange, ordered }: Props) {
  const ring = selected ? 'ring-2 ring-blue-500 ring-offset-app-bg' : 'ring-1 ring-transparent hover:ring-slate-200'
  ...
  return (
    <div className={baseWrap} onClick={onSelect} role="presentation">
      <div className={`${alignClass(block.align)}`}>
        <ListTag className={`${listClass} space-y-1 py-1 pl-5 text-sm text-app-label`}>
          {block.items.filter((x) => x.trim()).map((item, i) => (
            <li key={i}>{item}</li>
          ))}
          {!block.items.some((x) => x.trim()) && (
            <li className="text-app-tertiary">{ordered ? 'One line per item…' : 'One bullet per line…'}</li>
          )}
        </ListTag>
      </div>
      {selected && (
        <label className="mt-2 block text-xs font-medium text-app-secondary">
          {ordered ? 'Edit list (one per line)' : 'Edit bullets (one per line)'}
          <textarea
            className="mt-1 w-full rounded-md border border-app-separator bg-app-surface px-2 py-2 text-sm text-app-label focus:border-app-accent focus:outline-none focus:ring-2 focus:ring-app-accent/20"
            rows={4}
            value={value}
            onChange={(e) => {
